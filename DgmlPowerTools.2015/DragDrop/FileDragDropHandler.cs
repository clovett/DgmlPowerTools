...
namespace LovettSoftware.DgmlPowerTools
{
    ...
    public sealed class FileDragDropHandler : IGraphDropHandler
    {
        ...
        public bool CanDrop(object target, IDragEventArgs args)
        {
        ...
        public void OnDrop(object sender, IDragEventArgs args)
        {
            if (CanReceive(args.Data))
            {
        ...
        public int GetDragSourceCount(object target, IDragEventArgs args)
        {
            return GetFilesDropped(args.Data).Count();
        }
        ...
        {
            OnFilesDropped(control, target, pos, GetFilesDropped(data));
        }

        private void OnFilesDropped(GraphControl control, GraphNode target, Point pos, IEnumerable<string> files)
        {
            if (control == null)
            {
                return;
